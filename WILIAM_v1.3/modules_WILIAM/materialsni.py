...
@component.add(
    name="AMOUNT OF Ni IN WEIGHT OF ROCK",
    units="DMNL",
...
@component.add(
    name="COEFFICIENT Cu INVERTED COST CURVE",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="COEFFICIENTS GLOBAL Ni OTHER DEMAND",
    units="DMNL",
...
def coefficients_global_ni_other_demand():
    ...
    value.loc[["A S CURVE"]] = 4
    value.loc[["B S CURVE"]] = 0.000295113
    value.loc[["C S CURVE"]] = 11500
    return value


@component.add(
    name="COEFFICIENTS HIGH TECH Ni DEMAND",
    units="1/Years",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
@component.add(
    name="COEFFICIENTS Ni CHANGE GRADE TWO",
...
@component.add(
    name="COEFFICIENTS Ni ENERGY USE",
    units="MJ/kg",
    subscripts=["ORE GRADES I"],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="COEFFICIENTS Ni GRADE COST",
    units="M$/Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Constant",
    comp_subtype="Normal",
...
def coefficients_ni_grade_cost():
    ...
    value.loc[["RICH GRADE"]] = 4000
    value.loc[["HIGH GRADE"]] = 7000
    value.loc[["LOW GRADE"]] = 15000
    value.loc[["ULTRALOW GRADE"]] = 28000
    value.loc[["TRACE GRADE"]] = 40000
    value.loc[["OCEANS GRADE"]] = 0
    return value


@component.add(
    name="COEFFICIENTS Ni MINING EFFICIENCY CURVE",
    units="DMNL",
    subscripts=["S CURVE FIT ABC I"],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="COEFFICIENTS Ni OTHER RECYCLING FRACTION",
    units="DMNL",
    subscripts=["S CURVE FIT ABC I"],
    comp_type="Constant",
    comp_subtype="Normal",
...
def coefficients_ni_other_recycling_fraction():
    ...
    value.loc[["A S CURVE"]] = 0.9
    value.loc[["B S CURVE"]] = 0.000217753
    value.loc[["C S CURVE"]] = 5863.35
    return value


@component.add(
    name="COEFFICIENTS Ni PRICE EFFECT ON DEMAND",
    units="DMNL",
...
def coefficients_ni_price_effect_on_demand():
    ...
    value.loc[["A LINEAR LOG FIT"]] = 1.1501
    value.loc[["B LINEAR LOG FIT"]] = -1e-05
    return value


@component.add(
    name="coefficients ni price linear",
    units="DMNL",
    subscripts=["LINEAR LOG FIT I"],
    comp_type="Constant",
    comp_subtype="Normal",
...
def coefficients_ni_price_linear():
    ...
    value.loc[["A LINEAR LOG FIT"]] = -218667
    value.loc[["B LINEAR LOG FIT"]] = 48800
    return value


@component.add(
    name="coefficients ni price logistic",
    units="DMNL",
    subscripts=["LINEAR LOG FIT I"],
    comp_type="Constant",
    comp_subtype="Normal",
...
def coefficients_ni_price_logistic():
    ...
    value.loc[["A LINEAR LOG FIT"]] = 16000
    value.loc[["B LINEAR LOG FIT"]] = -0.763
    return value


@component.add(
    name="coefficients ni profit driver",
    units="DMNL",
    subscripts=["S CURVE FIT ABC I"],
    comp_type="Constant",
    comp_subtype="Normal",
...
def coefficients_ni_profit_driver():
    ...
    value.loc[["A S CURVE"]] = 1.25819
    value.loc[["B S CURVE"]] = 0.00012928
    value.loc[["C S CURVE"]] = 5268.4
    return value


@component.add(
    name="COEFFICIENTS PROSPECTING FROM GRADES TWO",
    subscripts=["ORE GRADES I", "S CURVE FIT REVERSE I"],
    comp_type="Constant",
    comp_subtype="External",
    depends_on={
...
@component.add(
    name="COEFFICIENTS PROSPECTING TECHNOLOGY IMPROVEMENTS TWO",
    subscripts=["ORE GRADES I", "S CURVE FIT ABC I"],
    comp_type="Constant",
    comp_subtype="External",
    depends_on={
...
@component.add(
    name="COEFFICIENTS PROSPECTONG FROM ALL TWO",
    subscripts=["ORE GRADES I", "S CURVE FIT REVERSE I"],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="COEFFICIENTS WATER PER Ni",
    units="Mt/Years",
    subscripts=["ORE GRADES I"],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="delayed TS Ni price economy adjusted",
    units="DMNL",
    comp_type="Stateful",
...
@component.add(
    name="delayed TS Ni profit average",
    units="DMNL",
    comp_type="Stateful",
    comp_subtype="Integ",
...
def demand_ni_base_year():
    """
    Nickel demand in the Base year 2015, (international nickel Study Group, 2016, p. A–1)
    """
    return 1.88


@component.add(
    name="Global Ni other demand",
    units="Mt/Mpersons",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def global_ni_other_demand():
    ...
    """
    return float(coefficients_global_ni_other_demand().loc["A S CURVE"]) / (
        1
        + np.exp(
            -float(coefficients_global_ni_other_demand().loc["B S CURVE"])
            * (
                imv_gdp_per_person()
                - float(coefficients_global_ni_other_demand().loc["C S CURVE"])
            )
        )
    )
...
@component.add(
    name="IMV Cu MINING EFFICIENCY CURVE",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"time": 1},
...
@component.add(
    name="IMV SS MnCrNi recycled two",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"time": 1},
...
@component.add(
    name="INITIAL DELAYED TS Ni PROFIT AVERAGE",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="INITIAL Ni CUMULATIVE MINING",
    units="Mt",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
@component.add(
    name="INITIAL Ni HIDDEN RESOURCES",
    units="Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    name="INITIAL Ni KNOWN RESERVES",
    units="Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Constant",
    comp_subtype="External",
...
def mining_cost_ni_ton():
    ...
    """
    return (
        ni_recycling() * float(ni_grade_cost().loc["HIGH GRADE"])
        + float(ni_grade_cost().loc["RICH GRADE"])
        * float(ni_mining().loc["RICH GRADE"])
        + float(ni_mining().loc["HIGH GRADE"])
        * float(ni_grade_cost().loc["HIGH GRADE"])
        + float(ni_mining().loc["LOW GRADE"]) * float(ni_grade_cost().loc["LOW GRADE"])
        + float(ni_mining().loc["ULTRALOW GRADE"])
        * float(ni_grade_cost().loc["ULTRALOW GRADE"])
        + float(ni_mining().loc["TRACE GRADE"])
        * float(ni_grade_cost().loc["TRACE GRADE"])
        + float(ni_mining().loc["OCEANS GRADE"])
        * float(ni_grade_cost().loc["OCEANS GRADE"])
    ) / (
        float(ni_mining().loc["RICH GRADE"])
        + float(ni_mining().loc["HIGH GRADE"])
        + float(ni_mining().loc["LOW GRADE"])
        + float(ni_mining().loc["ULTRALOW GRADE"])
        + float(ni_mining().loc["TRACE GRADE"])
        + float(ni_mining().loc["OCEANS GRADE"])
        + ni_recycling()
...
def mining_technology_improvements_for_ni():
    ...
    value.loc[["RICH GRADE"]] = 0
    value.loc[["HIGH GRADE"]] = 0
    value.loc[["LOW GRADE"]] = float(
        coefficients_mining_technology_improvements().loc["LOW GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(
    ...
    value.loc[["ULTRALOW GRADE"]] = float(
        coefficients_mining_technology_improvements().loc["ULTRALOW GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(
                coefficients_mining_technology_improvements().loc[
                    "ULTRALOW GRADE", "B S CURVE"
                ]
            )
            * (
                time()
    ...
    value.loc[["TRACE GRADE"]] = float(
        coefficients_mining_technology_improvements().loc["TRACE GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(
                coefficients_mining_technology_improvements().loc[
                    "TRACE GRADE", "B S CURVE"
                ]
            )
            * (
                time()
...
@component.add(
    name="Ni base price 2000", units="$/t", comp_type="Constant", comp_subtype="Normal"
)
def ni_base_price_2000():
    """
    Nickel price per ton in the year 2015.
    """
    return 2000
...
def ni_change_grade():
    ...
    value.loc[["HIGH GRADE"]] = float(
        coefficients_ni_change_grade().loc["HIGH GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(coefficients_ni_change_grade().loc["HIGH GRADE", "B S CURVE"])
            * (
                ni_price()
                - float(coefficients_ni_change_grade().loc["HIGH GRADE", "C S CURVE"])
            )
        )
    )
    value.loc[["LOW GRADE"]] = float(
        coefficients_ni_change_grade().loc["LOW GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(coefficients_ni_change_grade().loc["LOW GRADE", "B S CURVE"])
            * (
                ni_price()
                - float(coefficients_ni_change_grade().loc["LOW GRADE", "C S CURVE"])
            )
        )
    )
    value.loc[["ULTRALOW GRADE"]] = float(
        coefficients_ni_change_grade().loc["ULTRALOW GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(coefficients_ni_change_grade().loc["ULTRALOW GRADE", "B S CURVE"])
            * (
                ni_price()
                - float(
                    coefficients_ni_change_grade().loc["ULTRALOW GRADE", "C S CURVE"]
                )
            )
        )
    )
    value.loc[["TRACE GRADE"]] = float(
        coefficients_ni_change_grade().loc["TRACE GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(coefficients_ni_change_grade().loc["TRACE GRADE", "B S CURVE"])
            * (
                ni_price()
                - float(coefficients_ni_change_grade().loc["TRACE GRADE", "C S CURVE"])
            )
        )
    )
    value.loc[["OCEANS GRADE"]] = 0
...
@component.add(
    name="Ni CONTENT IN COMMON STEEL",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
@component.add(
    name="Ni CONTENT IN METALS",
    units="DMNL",
    subscripts=["NI CONTENT IN METALS I"],
    comp_type="Constant",
    comp_subtype="External",
...
_ext_constant_ni_content_in_metals = ExtConstant(
    ...
    {"NI CONTENT IN METALS I": _subscript_dict["NI CONTENT IN METALS I"]},
    _root,
    {"NI CONTENT IN METALS I": _subscript_dict["NI CONTENT IN METALS I"]},
...
def ni_energy_use():
    ...
    value.loc[["RICH GRADE"]] = float(
        coefficients_ni_energy_use().loc["RICH GRADE"]
    ) * float(ni_mining().loc["RICH GRADE"])
    value.loc[["HIGH GRADE"]] = float(ni_mining().loc["HIGH GRADE"]) * float(
        coefficients_ni_energy_use().loc["HIGH GRADE"]
    )
    value.loc[["LOW GRADE"]] = float(ni_mining().loc["LOW GRADE"]) * float(
        coefficients_ni_energy_use().loc["LOW GRADE"]
    )
    value.loc[["ULTRALOW GRADE"]] = float(ni_mining().loc["ULTRALOW GRADE"]) * float(
        coefficients_ni_energy_use().loc["ULTRALOW GRADE"]
    )
    value.loc[["TRACE GRADE"]] = float(ni_mining().loc["TRACE GRADE"]) * float(
        coefficients_ni_energy_use().loc["TRACE GRADE"]
    )
    value.loc[["OCEANS GRADE"]] = 0
    return value
...
@component.add(
    name="Ni energy use from extraction",
    units="TJ/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def ni_energy_use_from_extraction():
    ...
    return (
        (
            float(ni_energy_use().loc["HIGH GRADE"])
            + float(ni_energy_use().loc["LOW GRADE"])
            + float(ni_energy_use().loc["ULTRALOW GRADE"])
            + float(ni_energy_use().loc["TRACE GRADE"])
            + float(ni_energy_use().loc["RICH GRADE"])
        )
...
@component.add(
    name="Ni extraction rate coefficient",
    units="1/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def ni_find():
    ...
    value.loc[["RICH GRADE"]] = (
        float(ni_hidden_resources().loc["RICH GRADE"]) * ni_rich_find_rate()
    )
    value.loc[["HIGH GRADE"]] = (
        float(scaling_numbers_from_ni_hidden_to_ni_known().loc["HIGH GRADE"])
        * float(prospecting_for_ni_from_all().loc["HIGH GRADE"])
        * float(ni_hidden_resources().loc["HIGH GRADE"])
        * prospekting_rate_ni()
        * float(prospecting_technology_improvements_for_ni().loc["HIGH GRADE"])
    )
    value.loc[["LOW GRADE"]] = (
        float(scaling_numbers_from_ni_hidden_to_ni_known().loc["LOW GRADE"])
        * float(prospecting_for_ni_from_grades().loc["LOW GRADE"])
        * float(prospecting_technology_improvements_for_ni().loc["LOW GRADE"])
        * float(prospecting_for_ni_from_all().loc["LOW GRADE"])
        * float(ni_hidden_resources().loc["LOW GRADE"])
    )
    value.loc[["ULTRALOW GRADE"]] = (
        float(prospecting_for_ni_from_grades().loc["ULTRALOW GRADE"])
        * float(prospecting_technology_improvements_for_ni().loc["ULTRALOW GRADE"])
        * float(ni_hidden_resources().loc["ULTRALOW GRADE"])
        * prospekting_rate_ni()
    )
    value.loc[["TRACE GRADE"]] = (
        float(ni_hidden_resources().loc["TRACE GRADE"])
        * float(prospecting_technology_improvements_for_ni().loc["TRACE GRADE"])
        * float(prospecting_for_ni_from_grades().loc["TRACE GRADE"])
        * prospekting_rate_ni()
        * float(mining_technology_improvements_for_ni().loc["TRACE GRADE"])
    )
    value.loc[["OCEANS GRADE"]] = 0
    return value
...
def ni_from_cu():
    return float(ni_content_in_metals().loc["Ni IN Cu"]) * imv_cu_mined_0()
...
def ni_from_pgm():
    return (
        imv_pgm_from_mining()
        * float(ni_content_in_metals().loc["Ni IN PGM"])
        * unit_conversion_mt_kt()
    )


@component.add(
    name="Ni full recycling",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_grade_cost():
    ...
    value.loc[["RICH GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["RICH GRADE"])
        / ni_mining_efficiency_curve()
    )
    value.loc[["HIGH GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["HIGH GRADE"])
        / ni_mining_efficiency_curve()
    )
    value.loc[["LOW GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["LOW GRADE"])
        / ni_mining_efficiency_curve()
    )
    value.loc[["ULTRALOW GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["ULTRALOW GRADE"])
        / ni_mining_efficiency_curve()
    )
    value.loc[["TRACE GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["TRACE GRADE"])
        / ni_mining_efficiency_curve()
    )
    value.loc[["OCEANS GRADE"]] = (
        float(coefficients_ni_grade_cost().loc["OCEANS GRADE"])
        / ni_mining_efficiency_curve()
    )
    return value
...
@component.add(
    name="Ni HISTORICAL GLOBAL CONSUMPTION",
    units="Kt/Year",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"time": 1},
...
@component.add(
    name="Ni HISTORICAL GLOBAL CONSUMPTION Mt",
    units="Mt/Year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Ni known reserves all grades",
    units="Mt",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_known_reserves_all_grades():
    ...
    """
    return (
        float(ni_known_reserves().loc["RICH GRADE"])
        + float(ni_known_reserves().loc["HIGH GRADE"])
        + float(ni_known_reserves().loc["LOW GRADE"])
        + float(ni_known_reserves().loc["ULTRALOW GRADE"])
        + float(ni_known_reserves().loc["TRACE GRADE"])
        + float(ni_known_reserves().loc["OCEANS GRADE"])
    )
...
def ni_mining():
    ...
    value.loc[["RICH GRADE"]] = (
        float(ni_known_reserves().loc["RICH GRADE"]) * ni_extraction_rate_coefficient()
    )
    value.loc[["HIGH GRADE"]] = (
        ni_extraction_rate_coefficient()
        * float(ni_known_reserves().loc["HIGH GRADE"])
        * float(ni_change_grade().loc["HIGH GRADE"])
    )
    value.loc[["LOW GRADE"]] = (
        float(ni_known_reserves().loc["LOW GRADE"])
        * ni_extraction_rate_coefficient()
        * float(ni_change_grade().loc["LOW GRADE"])
        * float(mining_technology_improvements_for_ni().loc["LOW GRADE"])
    )
    value.loc[["ULTRALOW GRADE"]] = (
        float(ni_known_reserves().loc["ULTRALOW GRADE"])
        * ni_extraction_rate_coefficient()
        * float(ni_change_grade().loc["ULTRALOW GRADE"])
        * float(mining_technology_improvements_for_ni().loc["ULTRALOW GRADE"])
    )
    value.loc[["TRACE GRADE"]] = (
        float(ni_known_reserves().loc["TRACE GRADE"])
        * ni_extraction_rate_coefficient()
        * float(ni_change_grade().loc["TRACE GRADE"])
        * float(mining_technology_improvements_for_ni().loc["TRACE GRADE"])
    )
    value.loc[["OCEANS GRADE"]] = 0
    return value
...
@component.add(
    name="Ni mining efficiency curve",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_mining_efficiency_curve():
    ...
    """
    return float(coefficients_ni_mining_efficiency_curve().loc["A S CURVE"]) / (
        1
        + np.exp(
            -float(coefficients_ni_mining_efficiency_curve().loc["B S CURVE"])
            * (
                time()
                - float(coefficients_ni_mining_efficiency_curve().loc["C S CURVE"])
            )
        )
    )
...
def ni_mining_expences():
    ...
    """
    return (
        float(ni_grade_cost().loc["RICH GRADE"]) * float(ni_mining().loc["RICH GRADE"])
        + float(ni_grade_cost().loc["HIGH GRADE"])
        * float(ni_mining().loc["HIGH GRADE"])
        + float(ni_grade_cost().loc["LOW GRADE"]) * float(ni_mining().loc["LOW GRADE"])
        + float(ni_grade_cost().loc["ULTRALOW GRADE"])
        * float(ni_mining().loc["ULTRALOW GRADE"])
        + float(ni_grade_cost().loc["TRACE GRADE"])
        * float(ni_mining().loc["TRACE GRADE"])
        + float(ni_grade_cost().loc["OCEANS GRADE"])
        * float(ni_mining().loc["OCEANS GRADE"])
        + ni_secondary() * float(ni_grade_cost().loc["LOW GRADE"])
    )


@component.add(
    name="Ni mining Income",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Ni modified demand WORLD7",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def ni_ore_grade():
    ...
    return (
        float(ni_mining().loc["RICH GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["RICH GRADE"])
        + float(ni_mining().loc["HIGH GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["HIGH GRADE"])
        + float(ni_mining().loc["LOW GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["LOW GRADE"])
        + float(ni_mining().loc["ULTRALOW GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["ULTRALOW GRADE"])
        + float(ni_mining().loc["TRACE GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["TRACE GRADE"])
        + float(ni_mining().loc["OCEANS GRADE"])
        * float(amount_of_ni_in_weight_of_rock().loc["OCEANS GRADE"])
    ) / (
        float(ni_mining().loc["RICH GRADE"])
        + float(ni_mining().loc["HIGH GRADE"])
        + float(ni_mining().loc["LOW GRADE"])
...
@component.add(
    name="Ni other recycled fraction",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_other_recycled_fraction():
    ...
    Olafsdottir, A.H., and Sverdrup, H., 2020, System dynamics modelling of mining, supply, recycling, stocks-in-use and market price for nickel. Mining, Metallurgy & Exploration. 1-22. 10.1007/s42461-020-00370-
    """
    return float(coefficients_ni_other_recycling_fraction().loc["A S CURVE"]) / (
        1
        + np.exp(
            -float(coefficients_ni_other_recycling_fraction().loc["B S CURVE"])
            * (
                ni_price()
                - float(coefficients_ni_other_recycling_fraction().loc["C S CURVE"])
            )
        )
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"ni_supply_to_society": 1, "share_ni_other_supply": 1},
)
def ni_other_uses():
    return ni_supply_to_society() * share_ni_other_supply()


@component.add(
    name="Ni PLATED SCRAPPING RATE",
    units="1/Years",
    comp_type="Constant",
    comp_subtype="External",
...
def ni_price():
    ...
    return if_then_else(
        ...
        lambda: float(coefficients_ni_price_logistic().loc["A LINEAR LOG FIT"])
        * np.exp(
            float(coefficients_ni_price_logistic().loc["B LINEAR LOG FIT"])
            * ni_market()
        ),
        lambda: float(coefficients_ni_price_linear().loc["A LINEAR LOG FIT"])
        * ni_market()
        + float(coefficients_ni_price_linear().loc["B LINEAR LOG FIT"]),
    )


@component.add(
    name="Ni price economy",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_price_economy():
    """
    Nickel price for the economy module.
    """
    return if_then_else(
...
@component.add(
    name="Ni price economy adjusted",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_price_economy_adjusted():
    """
    Nickel price, adjusted for the economy model.
    """
    return if_then_else(time() == 2005, lambda: 100, lambda: ni_price_index_economy())


@component.add(
    name="Ni price effect on demand",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_price_effect_on_demand():
    ...
    """
    return float(
        coefficients_ni_price_effect_on_demand().loc["A LINEAR LOG FIT"]
    ) * np.exp(
        float(coefficients_ni_price_effect_on_demand().loc["B LINEAR LOG FIT"])
        * ni_price()
    )


@component.add(
    name="Ni price index economy",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def ni_price_index_economy():
    """
    Nickel price conversion from $/t to the reference price signal used in the economy module.
    """
    return if_then_else(
        switch_mat2eco_ni_price() == 0,
        lambda: 100,
        lambda: (
            float(estimated_price_with_tax_metals().loc["Ni W"]) / ni_base_price_2000()
        )
...
def ni_profit_driver():
    ...
    return if_then_else(
        ni_mining_profit() < 0,
        lambda: 0,
        lambda: float(coefficients_ni_profit_driver().loc["A S CURVE"])
        / (
            1
            + np.exp(
                -float(coefficients_ni_profit_driver().loc["B S CURVE"])
                * (
                    ni_mining_profit()
                    - float(coefficients_ni_profit_driver().loc["C S CURVE"])
                )
            )
        ),
    )
...
@component.add(
    name="Ni SCRAPPING SHARE RECYCLING SP",
    units="1/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_scrapping_share_recycling_sp():
    ...
    """
    return float(eol_recycling_rates_minerals_base_metals().loc["Ni W"])
...
@component.add(
    name="Ni share of secondary material",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def ni_total_mining():
    ...
    """
    return sum(
        ni_mining().rename({"ORE GRADES I": "ORE GRADES I!"}), dim=["ORE GRADES I!"]
    )


@component.add(
    name="Ni UNITS PER Cu USED",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
def prospecting_for_ni_from_all():
    ...
    value.loc[["RICH GRADE"]] = float(
        coefficients_prospecting_from_all().loc["N", "RICH GRADE"]
    )
    value.loc[["HIGH GRADE"]] = float(
        coefficients_prospecting_from_all().loc["MIN VALUE", "HIGH GRADE"]
    ) + (
        float(coefficients_prospecting_from_all().loc["MAX VALUE", "HIGH GRADE"])
        - float(coefficients_prospecting_from_all().loc["MIN VALUE", "HIGH GRADE"])
    ) / (
        1
        + 10
        ** (
            float(coefficients_prospecting_from_all().loc["N", "HIGH GRADE"])
            * (
    ...
    value.loc[["LOW GRADE"]] = float(
        coefficients_prospecting_from_all().loc["MIN VALUE", "LOW GRADE"]
    ) + (
        float(coefficients_prospecting_from_all().loc["MAX VALUE", "LOW GRADE"])
        - float(coefficients_prospecting_from_all().loc["MIN VALUE", "LOW GRADE"])
    ) / (
        1
        + 10
        ** (
            float(coefficients_prospecting_from_all().loc["N", "LOW GRADE"])
            * (
                (np.log(ni_known_reserves_all_grades()) / np.log(10))
                - (
                    np.log(
                        float(
                            coefficients_prospecting_from_all().loc["EC50", "LOW GRADE"]
                        )
                    )
                    / np.log(10)
                )
    ...
    value.loc[["ULTRALOW GRADE"]] = float(
        coefficients_prospecting_from_all().loc["MIN VALUE", "ULTRALOW GRADE"]
    ) + (
        float(coefficients_prospecting_from_all().loc["MAX VALUE", "ULTRALOW GRADE"])
        - float(coefficients_prospecting_from_all().loc["MIN VALUE", "ULTRALOW GRADE"])
    ) / (
        1
        + 10
        ** (
            float(coefficients_prospecting_from_all().loc["N", "ULTRALOW GRADE"])
            * (
                (np.log(ni_known_reserves_all_grades()) / np.log(10))
                - (
                    np.log(
    ...
    value.loc[["TRACE GRADE"]] = float(
        coefficients_prospecting_from_all().loc["MIN VALUE", "TRACE GRADE"]
    ) + (
        float(coefficients_prospecting_from_all().loc["MAX VALUE", "TRACE GRADE"])
        - float(coefficients_prospecting_from_all().loc["MIN VALUE", "TRACE GRADE"])
    ) / (
        1
        + 10
        ** (
            float(coefficients_prospecting_from_all().loc["N", "TRACE GRADE"])
            * (
                (np.log(ni_known_reserves_all_grades()) / np.log(10))
                - (
                    np.log(
...
def prospecting_for_ni_from_grades():
    ...
    value.loc[["LOW GRADE"]] = if_then_else(
        float(ni_known_reserves().loc["LOW GRADE"]) > 0,
        lambda: float(
            coefficients_prospecting_from_grades().loc["MIN VALUE", "LOW GRADE"]
        )
        + (
            float(coefficients_prospecting_from_grades().loc["MAX VALUE", "LOW GRADE"])
            - float(
                coefficients_prospecting_from_grades().loc["MIN VALUE", "LOW GRADE"]
            )
        )
        / (
            ...
            + 10
            ** (
                float(coefficients_prospecting_from_grades().loc["N", "LOW GRADE"])
                * (
                    (np.log(float(ni_known_reserves().loc["LOW GRADE"])) / np.log(10))
                    - (
                        np.log(
                            float(
    ...
    value.loc[["ULTRALOW GRADE"]] = if_then_else(
        float(ni_known_reserves().loc["ULTRALOW GRADE"]) > 0,
        lambda: float(
            coefficients_prospecting_from_grades().loc["MIN VALUE", "ULTRALOW GRADE"]
        )
        + (
            float(
                coefficients_prospecting_from_grades().loc[
                    "MAX VALUE", "ULTRALOW GRADE"
                ]
            )
            - float(
                coefficients_prospecting_from_grades().loc[
                    "MIN VALUE", "ULTRALOW GRADE"
                ]
            )
        )
        / (
            ...
            + 10
            ** (
                float(coefficients_prospecting_from_grades().loc["N", "ULTRALOW GRADE"])
                * (
                    (
                        np.log(float(ni_known_reserves().loc["ULTRALOW GRADE"]))
                        / np.log(10)
                    )
                    - (
                        np.log(
    ...
    value.loc[["TRACE GRADE"]] = if_then_else(
        float(ni_known_reserves().loc["TRACE GRADE"]) > 0,
        lambda: float(
            coefficients_prospecting_from_grades().loc["MIN VALUE", "TRACE GRADE"]
        )
        + (
            float(
                coefficients_prospecting_from_grades().loc["MAX VALUE", "TRACE GRADE"]
            )
            - float(
                coefficients_prospecting_from_grades().loc["MIN VALUE", "TRACE GRADE"]
            )
        ...
        / (
            ...
            + 10
            ** (
                float(coefficients_prospecting_from_grades().loc["N", "TRACE GRADE"])
                * (
                    (np.log(float(ni_known_reserves().loc["TRACE GRADE"])) / np.log(10))
                    - (
                        np.log(
                            float(
                                coefficients_prospecting_from_grades().loc[
...
def prospecting_technology_improvements_for_ni():
    ...
    value.loc[["LOW GRADE"]] = float(
        coefficients_prospecting_technology_improvements().loc["LOW GRADE", "A S CURVE"]
    ) / (
        1
        + np.exp(
            -float(
...
@component.add(
    name="SHARE Ni OTHER SUPPLY", comp_type="Constant", comp_subtype="Normal"
)
def share_ni_other_supply():
    """
    Share of Nickel that flows into other uses.
    """
    return 0.08


@component.add(
    name="SWITCH ECO2MAT Ni DEMAND",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
...
def total_ni_known_reserves():
    ...
    """
    return sum(
        ni_known_reserves().rename({"ORE GRADES I": "ORE GRADES I!"}),
        dim=["ORE GRADES I!"],
    )
...
def water_ni_extraction():
    ...
    """
    return (
        float(water_use_ni().loc["TRACE GRADE"])
        + float(water_use_ni().loc["ULTRALOW GRADE"])
        + float(water_use_ni().loc["LOW GRADE"])
        + float(water_use_ni().loc["HIGH GRADE"])
        + float(water_use_ni().loc["RICH GRADE"])
...
def water_per_ni():
    ...
    value.loc[["RICH GRADE"]] = float(coefficients_water_per_ni().loc["RICH GRADE"])
    value.loc[["HIGH GRADE"]] = float(coefficients_water_per_ni().loc["HIGH GRADE"])
    value.loc[["LOW GRADE"]] = float(coefficients_water_per_ni().loc["LOW GRADE"])
    value.loc[["ULTRALOW GRADE"]] = float(
        coefficients_water_per_ni().loc["ULTRALOW GRADE"]
    )
    value.loc[["TRACE GRADE"]] = float(coefficients_water_per_ni().loc["TRACE GRADE"])
    value.loc[["OCEANS GRADE"]] = 0
    return value


@component.add(
    name="WATER PER Ni RECYCLE",
    units="Mm3/Mt",
    comp_type="Constant",
    comp_subtype="External",
...
def water_use_ni():
    ...
    value.loc[["RICH GRADE"]] = float(ni_mining().loc["RICH GRADE"]) * float(
        water_per_ni().loc["RICH GRADE"]
    )
    value.loc[["HIGH GRADE"]] = float(ni_mining().loc["HIGH GRADE"]) * float(
        water_per_ni().loc["HIGH GRADE"]
    )
    value.loc[["LOW GRADE"]] = float(water_per_ni().loc["LOW GRADE"]) * float(
        ni_mining().loc["LOW GRADE"]
    )
    value.loc[["ULTRALOW GRADE"]] = float(ni_mining().loc["ULTRALOW GRADE"]) * float(
        water_per_ni().loc["ULTRALOW GRADE"]
    )
    value.loc[["TRACE GRADE"]] = float(water_per_ni().loc["TRACE GRADE"]) * float(
        ni_mining().loc["TRACE GRADE"]
    )
    value.loc[["OCEANS GRADE"]] = 0
    return value
