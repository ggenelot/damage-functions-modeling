...
@component.add(
    name="a lineal regr rr base metals",
    units="DMNL",
...
def a_lineal_regr_rr_base_metals():
    ...
    """
    return (
        target_rr_rest_sp_w_base_metals() - current_eol_rr_minerals_base_metals()
    ) / (
        target_year_p_rr_minerals_rest_w_base_metals()
        - start_year_p_rr_minerals_w_base_metals()
    )


@component.add(
    name="AVOID DIVIDING WITH ZERO",
    units="DMNL",
    comp_type="Constant",
...
@component.add(
    name="b lineal regr rr base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def b_lineal_regr_rr_base_metals():
    ...
    """
    return (
        target_rr_rest_sp_w_base_metals()
        - a_lineal_regr_rr_base_metals()
        * target_year_p_rr_minerals_rest_w_base_metals()
    )


@component.add(
    name="by mineral rr 1yr base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Stateful",
    comp_subtype="DelayFixed",
    depends_on={"_delayfixed_by_mineral_rr_1yr_base_metals": 1},
    other_deps={
        "_delayfixed_by_mineral_rr_1yr_base_metals": {
            "initial": {"current_recycling_rates_minerals_alt_techn_base_metals": 1},
            "step": {"by_mineral_rr_base_metals": 1},
        }
    },
)
def by_mineral_rr_1yr_base_metals():
    """
    Recycling rates over time delayed 1 year by mineral for the rest of the economy.
    """
    return _delayfixed_by_mineral_rr_1yr_base_metals()


_delayfixed_by_mineral_rr_1yr_base_metals = DelayFixed(
    lambda: by_mineral_rr_base_metals(),
    lambda: 1,
    lambda: current_recycling_rates_minerals_alt_techn_base_metals(),
    time_step,
    "_delayfixed_by_mineral_rr_1yr_base_metals",
)
...
@component.add(
    name="by mineral rr base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "a_lineal_regr_rr_base_metals": 1,
        "time": 1,
        "b_lineal_regr_rr_base_metals": 1,
    },
)
def by_mineral_rr_base_metals():
    """
    Recycling rates over time by mineral for the rest of the economy.
    """
    return a_lineal_regr_rr_base_metals() * time() + b_lineal_regr_rr_base_metals()


@component.add(
    name="by mineral rr variation metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def by_mineral_rr_variation_metals():
    ...
    return if_then_else(
        time() < 2015,
        lambda: xr.DataArray(
            historic_improvement_recycling_rates_minerals_base_metals(),
            {"METALS W I": _subscript_dict["METALS W I"]},
            ["METALS W I"],
        ),
        lambda: if_then_else(
            time() < start_year_p_rr_minerals_w_base_metals(),
            lambda: xr.DataArray(
                historic_improvement_recycling_rates_minerals_base_metals(),
                {"METALS W I": _subscript_dict["METALS W I"]},
                ["METALS W I"],
            ),
            lambda: by_mineral_rr_base_metals() - by_mineral_rr_1yr_base_metals(),
        ),
    )


@component.add(
    name="change Cu grade",
    units="DMNL",
...
def change_cu_grade():
    ...
    """
    value = xr.DataArray(
        np.nan, {"ORE GRADES I": _subscript_dict["ORE GRADES I"]}, ["ORE GRADES I"]
    )
    value.loc[["RICH GRADE"]] = float(
        coefficients_change_cu_grade().loc["RICH GRADE", "A S CURVE FIT"]
    )
    value.loc[["HIGH GRADE"]] = if_then_else(
        cu_price_per_ton_share() > 0,
        lambda: float(coefficients_change_cu_grade().loc["HIGH GRADE", "A S CURVE FIT"])
        * (
            1
            - np.exp(
                -float(
                    coefficients_change_cu_grade().loc["HIGH GRADE", "k S CURVE FIT"]
                )
                * cu_price_per_ton_share()
                ** float(
                    coefficients_change_cu_grade().loc["HIGH GRADE", "n S CURVE FIT"]
                )
            )
        ),
        lambda: 0,
    )
    value.loc[["LOW GRADE"]] = if_then_else(
        cu_price_per_ton_share() > 0,
        lambda: float(coefficients_change_cu_grade().loc["LOW GRADE", "A S CURVE FIT"])
        * (
            1
            - np.exp(
                -float(coefficients_change_cu_grade().loc["LOW GRADE", "k S CURVE FIT"])
                * cu_price_per_ton_share()
                ** float(
                    coefficients_change_cu_grade().loc["LOW GRADE", "n S CURVE FIT"]
                )
            )
        ),
        lambda: 0,
    )
    value.loc[["ULTRALOW GRADE"]] = if_then_else(
        cu_price_per_ton_share() > 0,
        lambda: float(
            coefficients_change_cu_grade().loc["ULTRALOW GRADE", "A S CURVE FIT"]
        )
        * (
            1
            - np.exp(
                -float(
                    coefficients_change_cu_grade().loc[
                        "ULTRALOW GRADE", "k S CURVE FIT"
                    ]
                )
                * cu_price_per_ton_share()
                ** float(
                    coefficients_change_cu_grade().loc[
                        "ULTRALOW GRADE", "n S CURVE FIT"
                    ]
                )
            )
        ),
    ...
    value.loc[["TRACE GRADE"]] = if_then_else(
        cu_price_per_ton_share() > 0,
        lambda: float(
            coefficients_change_cu_grade().loc["TRACE GRADE", "A S CURVE FIT"]
        )
        * (
            1
            - np.exp(
                -float(
                    coefficients_change_cu_grade().loc["TRACE GRADE", "k S CURVE FIT"]
                )
                * cu_price_per_ton_share()
                ** float(
                    coefficients_change_cu_grade().loc["TRACE GRADE", "n S CURVE FIT"]
                )
            )
        ),
        lambda: 0,
    )
    value.loc[["OCEANS GRADE"]] = float(
        coefficients_change_cu_grade().loc["OCEANS GRADE", "A S CURVE FIT"]
    )
    return value


@component.add(
    name="change Cu mining technology S curve",
    units="DMNL",
    subscripts=["ORE GRADES I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def change_cu_mining_technology_s_curve():
    ...
    value.loc[["RICH GRADE"]] = float(
        coefficients_change_cu_mining_technology_s_curve().loc[
            "RICH GRADE", "A S CURVE FIT"
        ]
    )
    value.loc[["HIGH GRADE"]] = float(
        coefficients_change_cu_mining_technology_s_curve().loc[
            "HIGH GRADE", "A S CURVE FIT"
        ]
    )
    value.loc[["LOW GRADE"]] = float(
        coefficients_change_cu_mining_technology_s_curve().loc[
            "LOW GRADE", "A S CURVE FIT"
        ]
    )
    value.loc[["ULTRALOW GRADE"]] = float(
        coefficients_change_cu_mining_technology_s_curve().loc[
            "ULTRALOW GRADE", "A S CURVE FIT"
        ]
    ) * (
        1
        - np.exp(
    ...
    value.loc[["TRACE GRADE"]] = float(
        coefficients_change_cu_mining_technology_s_curve().loc[
            "TRACE GRADE", "A S CURVE FIT"
        ]
    ) * (
        1
        - np.exp(
            -float(
                coefficients_change_cu_mining_technology_s_curve().loc[
                    "TRACE GRADE", "k S CURVE FIT"
                ]
            )
            * (time() - imv_start_time())
            ** float(
                coefficients_change_cu_mining_technology_s_curve().loc[
                    "TRACE GRADE", "n S CURVE FIT"
                ]
            )
        )
    )
    value.loc[["OCEANS GRADE"]] = float(
        ...
        - np.exp(
            -float(
                coefficients_change_cu_mining_technology_s_curve().loc[
                    "OCEANS GRADE", "k S CURVE FIT"
                ]
            )
            * (time() - imv_start_time())
            ** float(
                coefficients_change_cu_mining_technology_s_curve().loc[
                    "OCEANS GRADE", "n S CURVE FIT"
                ]
            )
        )
    )
...
@component.add(
    name="CHILE TOTAL ENERGY CONSUMPTION OF Cu MINING HISTORICAL",
    units="TJ/Years",
    comp_type="Auxiliary",
...
@component.add(
    name="COEFFICIENTS Cu PRICE ECONOMY",
    units="DMNL",
...
@component.add(
    name="common rr minerals variation base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "time": 1,
        "start_year_p_common_rr_minerals_w_base_metals": 1,
        "historic_improvement_recycling_rates_minerals_base_metals": 1,
        "policy_common_rr_minerals_variation_sp_w_base_metals": 1,
    },
)
def common_rr_minerals_variation_base_metals():
    """
    Recycling rates of minererals (common annual variation).
    """
    return xr.DataArray(
        if_then_else(
            time() < start_year_p_common_rr_minerals_w_base_metals(),
            lambda: historic_improvement_recycling_rates_minerals_base_metals(),
            lambda: policy_common_rr_minerals_variation_sp_w_base_metals(),
        ),
        {"METALS W I": _subscript_dict["METALS W I"]},
        ["METALS W I"],
    )
...
@component.add(
    name="constrain rr improv for per mineral base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        ...
        "max_recycling_rates_minerals_base_metals": 1,
    },
)
def constrain_rr_improv_for_per_mineral_base_metals():
    """
    Remaining recycling rate improvement for the rest of the economy per material.
    """
    return if_then_else(
        eol_recycling_rates_minerals_base_metals()
        < max_recycling_rates_minerals_base_metals(),
        lambda: xr.DataArray(
            1, {"METALS W I": _subscript_dict["METALS W I"]}, ["METALS W I"]
        ),
        lambda: xr.DataArray(
            0, {"METALS W I": _subscript_dict["METALS W I"]}, ["METALS W I"]
        ),
    )
...
@component.add(
    name="Cu AVAILABLE DELAYED INITIAL",
    units="Mt/Year",
    comp_type="Constant",
    comp_subtype="External",
...
def cu_cost_grade():
    ...
    value.loc[["RICH GRADE"]] = (
        float(cu_cost_grade_constants().loc["RICH GRADE"])
        / cu_mining_efficiency_curve()
    )
    value.loc[["HIGH GRADE"]] = (
        float(cu_cost_grade_constants().loc["HIGH GRADE"])
        / cu_mining_efficiency_curve()
    )
    value.loc[["LOW GRADE"]] = (
        float(cu_cost_grade_constants().loc["LOW GRADE"]) / cu_mining_efficiency_curve()
    )
    value.loc[["ULTRALOW GRADE"]] = (
        float(cu_cost_grade_constants().loc["ULTRALOW GRADE"])
        / cu_mining_efficiency_curve()
    )
    value.loc[["TRACE GRADE"]] = (
        float(cu_cost_grade_constants().loc["TRACE GRADE"])
        / cu_mining_efficiency_curve()
    )
    value.loc[["OCEANS GRADE"]] = (
        float(cu_cost_grade_constants().loc["OCEANS GRADE"])
        / cu_mining_efficiency_curve()
    )
    return value
...
@component.add(
    name='"Cu demand incl. semi product demand"',
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Cu DEMAND SCALING FACTOR FOR SEMI PRODUCTS",
    units="1/Year",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
def cu_energy_use():
    ...
    value.loc[["RICH GRADE"]] = float(
        coefficients_cu_energy_use().loc["RICH GRADE"]
    ) * float(mining_cu_known_reserves().loc["RICH GRADE"])
    value.loc[["HIGH GRADE"]] = float(
        mining_cu_known_reserves().loc["HIGH GRADE"]
    ) * float(coefficients_cu_energy_use().loc["HIGH GRADE"])
    value.loc[["LOW GRADE"]] = float(
        mining_cu_known_reserves().loc["LOW GRADE"]
    ) * float(coefficients_cu_energy_use().loc["LOW GRADE"])
    value.loc[["ULTRALOW GRADE"]] = float(
        mining_cu_known_reserves().loc["ULTRALOW GRADE"]
    ) * float(coefficients_cu_energy_use().loc["ULTRALOW GRADE"])
    value.loc[["TRACE GRADE"]] = float(
        mining_cu_known_reserves().loc["TRACE GRADE"]
    ) * float(coefficients_cu_energy_use().loc["TRACE GRADE"])
    value.loc[["OCEANS GRADE"]] = 0
    return value


@component.add(
    name="Cu energy use recycling",
    units="MJ/kg*Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Cu EOL RECYCLING RATE SP",
    units="1/Year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"eol_recycling_rates_minerals_base_metals": 1},
)
def cu_eol_recycling_rate_sp():
    ...
    """
    return float(eol_recycling_rates_minerals_base_metals().loc["Cu W"])
...
@component.add(
    name="Cu extraction coefficient",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def cu_from_ag():
    ...
    return (
        imv_ag_mining()
        * float(cu_content_in_metals().loc["Cu IN Ag"])
...
def cu_from_mo():
    ...
    """
    return (
        float(cu_content_in_metals().loc["Cu IN Mo"])
        * imv_mo_total_mining()
        * cu_mo_mines_with_cu()
    )
...
def cu_from_ni():
    ...
    """
    return imv_ni_total_mining() * float(cu_content_in_metals().loc["Cu IN Ni"])
...
def cu_from_pgm():
    ...
    return (
        imv_pgm_from_mining()
        * float(cu_content_in_metals().loc["Cu IN PGM"])
        * unit_conversion_mt_kt()
    )
...
def cu_from_zn():
    ...
    """
    return float(cu_content_in_metals().loc["Cu IN Zn"]) * imv_zn_mined()


@component.add(
    name="Cu hidden resources",
    units="Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Stateful",
    comp_subtype="Integ",
...
@component.add(
    name="Cu hidden resources to Cu known reserves",
    units="Mt/Years",
    subscripts=["ORE GRADES I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_hidden_resources_to_cu_known_reserves():
    ...
    value.loc[["RICH GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["RICH GRADE"])
        * cu_prospecting()
        * cu_prospecting_rich(),
        0,
    )
    value.loc[["HIGH GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["HIGH GRADE"]) * cu_prospecting(), 0
    )
    value.loc[["LOW GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["LOW GRADE"]) * cu_prospecting(), 0
    )
    value.loc[["ULTRALOW GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["ULTRALOW GRADE"])
        * cu_prospecting()
        * float(change_cu_mining_technology_s_curve().loc["ULTRALOW GRADE"]),
        0,
    )
    value.loc[["TRACE GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["TRACE GRADE"])
        * cu_prospecting()
        * float(change_cu_mining_technology_s_curve().loc["TRACE GRADE"]),
        0,
    )
    value.loc[["OCEANS GRADE"]] = np.maximum(
        float(cu_hidden_resources().loc["OCEANS GRADE"])
        * cu_prospecting()
        * float(change_cu_mining_technology_s_curve().loc["OCEANS GRADE"]),
        0,
    )
    return value
...
@component.add(
    name="Cu in use in society to Cu scrapping",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_income_ocean():
    ...
    """
    return float(mining_cu_known_reserves().loc["OCEANS GRADE"]) * cu_price_economy()


@component.add(
    name="Cu known reserves",
    units="Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Stateful",
    comp_subtype="Integ",
...
@component.add(
    name="cu losses during production",
    units="Mt/Year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_market_price_proxy():
    ...
    return if_then_else(
        cu_market() <= 0,
        lambda: float(coefficients_cu_market_price().loc["A LINEAR LOG FIT"])
        * np.log(0.1)
        + float(coefficients_cu_market_price().loc["B LINEAR LOG FIT"]),
        lambda: float(coefficients_cu_market_price().loc["A LINEAR LOG FIT"])
        * np.log(cu_market())
        + float(coefficients_cu_market_price().loc["B LINEAR LOG FIT"]),
    )
...
@component.add(
    name="Cu MINE PRODUCTION HISTORICAL",
    units="ton/Years",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"time": 1},
...
@component.add(
    name="Cu MINE PRODUCTION HISTORICAL MT",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_mined():
    ...
    return (
        float(mining_cu_known_reserves().loc["RICH GRADE"])
        * float(cu_yields().loc["RICH GRADE"])
        + float(mining_cu_known_reserves().loc["HIGH GRADE"])
        * float(cu_yields().loc["HIGH GRADE"])
        + float(mining_cu_known_reserves().loc["LOW GRADE"])
        * float(cu_yields().loc["LOW GRADE"])
        + float(mining_cu_known_reserves().loc["ULTRALOW GRADE"])
        * float(cu_yields().loc["ULTRALOW GRADE"])
        + float(mining_cu_known_reserves().loc["TRACE GRADE"])
        * float(cu_yields().loc["TRACE GRADE"])
        + float(mining_cu_known_reserves().loc["OCEANS GRADE"])
        * float(cu_yields().loc["OCEANS GRADE"])
    )


@component.add(
    name="Cu mined and Cu recycled EOL",
    units="Mt/Year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_mining_cost():
    ...
    """
    return (
        float(cu_primary_mining_cost().loc["RICH GRADE"])
        + float(cu_primary_mining_cost().loc["HIGH GRADE"])
        + float(cu_primary_mining_cost().loc["LOW GRADE"])
        + float(cu_primary_mining_cost().loc["ULTRALOW GRADE"])
        + float(cu_primary_mining_cost().loc["TRACE GRADE"])
        + float(cu_primary_mining_cost().loc["OCEANS GRADE"])
    )


@component.add(
    name="Cu mining efficiency curve",
    units="Mt",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_mining_efficiency_curve():
    ...
    """
    return float(
        coefficients_cu_mining_efficiency_curve().loc["A LINEAR LOG FIT"]
    ) * time() - float(
        coefficients_cu_mining_efficiency_curve().loc["B LINEAR LOG FIT"]
    )


@component.add(
    name="Cu mining income",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_ore_grade():
    ...
    return (
        float(mining_cu_known_reserves().loc["RICH GRADE"])
        * float(amount_of_cu_in_weight_of_rock().loc["RICH GRADE"])
        + float(mining_cu_known_reserves().loc["HIGH GRADE"])
        * float(amount_of_cu_in_weight_of_rock().loc["HIGH GRADE"])
        + float(mining_cu_known_reserves().loc["LOW GRADE"])
        * float(amount_of_cu_in_weight_of_rock().loc["LOW GRADE"])
        + float(mining_cu_known_reserves().loc["ULTRALOW GRADE"])
        * float(amount_of_cu_in_weight_of_rock().loc["ULTRALOW GRADE"])
        + float(mining_cu_known_reserves().loc["TRACE GRADE"])
        * float(amount_of_cu_in_weight_of_rock().loc["TRACE GRADE"])
    ) / (
        float(mining_cu_known_reserves().loc["RICH GRADE"])
        + float(mining_cu_known_reserves().loc["HIGH GRADE"])
        + float(mining_cu_known_reserves().loc["LOW GRADE"])
...
def cu_price_economy():
    ...
    return if_then_else(
        ...
        lambda: max_cu_price(),
        lambda: np.exp(
            float(coefficients_cu_price_economy().loc["A LINEAR LOG FIT"])
            + float(coefficients_cu_price_economy().loc["B LINEAR LOG FIT"])
...
@component.add(
    name="Cu price economy adjusted",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def cu_price_index_economy():
    ...
    return if_then_else(
        ...
        lambda: 100,
        lambda: (
            float(estimated_price_with_tax_metals().loc["Cu W"]) / cu_base_price_2015()
        )
...
def cu_profit_ocean():
    ...
    """
    return cu_income_ocean() - float(cu_primary_mining_cost().loc["OCEANS GRADE"])


@component.add(
    name="Cu profit push mining",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def cu_profit_push_mining():
    ...
    """
    return (
        float(cu_profit_push_mining_polynomal_fit().loc["SIXTH ORDER FIT"])
        * decrease_delayed_ts_cu_profit() ** 6
        + float(cu_profit_push_mining_polynomal_fit().loc["FIFTH ORDER FIT"])
        * decrease_delayed_ts_cu_profit() ** 5
        - float(cu_profit_push_mining_polynomal_fit().loc["FOURTH ORDER FIT"])
        * decrease_delayed_ts_cu_profit() ** 4
        - float(cu_profit_push_mining_polynomal_fit().loc["THIRD ORDER FIT"])
        * decrease_delayed_ts_cu_profit() ** 3
        + float(cu_profit_push_mining_polynomal_fit().loc["SECOND ORDER FIT"])
        * decrease_delayed_ts_cu_profit() ** 2
        + float(cu_profit_push_mining_polynomal_fit().loc["FIRST ORDER FIT"])
        * decrease_delayed_ts_cu_profit()
        + float(cu_profit_push_mining_polynomal_fit().loc["ZERO ORDER FIT"])
    )


@component.add(
    name="Cu profit push ocean",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def cu_profit_push_ocean():
    ...
    """
    return float(coefficients_cu_profit_push_ocean().loc["A S CURVE FIT"]) * (
        1
        - np.exp(
            -float(coefficients_cu_profit_push_ocean().loc["k S CURVE FIT"])
            * fraction_of_the_decrease_cu_ocean_profit_delay()
            ** float(coefficients_cu_profit_push_ocean().loc["n S CURVE FIT"])
        )
    )


@component.add(
    name="Cu RATE OF NEW SCRAP",
    units="Mt/Years",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
def cu_scrap_coefficient():
    ...
    """
    return float(coefficients_cu_scrap_coefficient().loc["A S CURVE"]) / (
        1
        + np.exp(
            -float(coefficients_cu_scrap_coefficient().loc["B S CURVE"])
            * (
                cu_price_economy()
                - float(coefficients_cu_scrap_coefficient().loc["C S CURVE"])
            )
        )
    )
...
def cu_scrapping_amplifier():
    ...
    """
    return float(coefficients_cu_scrapping().loc["A S CURVE"]) / (
        1
        + np.exp(
            -float(coefficients_cu_scrapping().loc["B S CURVE"])
            * (cu_price_economy() - float(coefficients_cu_scrapping().loc["C S CURVE"]))
        )
    )
...
def cu_secondary_cost():
    ...
    """
    return float(cu_cost_grade().loc["LOW GRADE"]) * cu_secondary()


@component.add(
    name="Cu secondary profits",
    units="M$",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Cu share entering recycling",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Cu share of secondary material",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="Cu total energy use from extraction and refining",
    units="TJ/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def cu_total_energy_use_from_extraction_and_refining():
    ...
    return (
        (
            float(cu_energy_use().loc["HIGH GRADE"])
            + float(cu_energy_use().loc["LOW GRADE"])
            + float(cu_energy_use().loc["ULTRALOW GRADE"])
            + float(cu_energy_use().loc["TRACE GRADE"])
            + cu_energy_use_recycling()
            + float(cu_energy_use().loc["RICH GRADE"])
...
@component.add(
    name="CURRENT RECYCLING RATES MINERALS ALT TECHN base metals",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "current_eol_rr_minerals_base_metals": 1,
        "rr_minerals_alt_techn_res_vs_total_economy_base_metals": 1,
    },
)
def current_recycling_rates_minerals_alt_techn_base_metals():
    return (
        current_eol_rr_minerals_base_metals()
        * rr_minerals_alt_techn_res_vs_total_economy_base_metals()
    )


@component.add(
    name="decrease Cu in use in society",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="decrease delayed TS Cu ocean profit delay",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="decrease delayed TS Cu profit",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="DELAYED Cu price economy adjusted test",
    units="$/t",
    comp_type="Stateful",
    comp_subtype="DelayFixed",
...
@component.add(
    name="delayed TS Cu ocean profit",
    units="M$",
    comp_type="Stateful",
    comp_subtype="Integ",
...
@component.add(
    name="delayed TS Cu price economy adjusted",
    units="DMNL",
    comp_type="Stateful",
    comp_subtype="DelayFixed",
...
@component.add(
    name="EOL recycling rates minerals base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Stateful",
    comp_subtype="Integ",
    depends_on={"_integ_eol_recycling_rates_minerals_base_metals": 1},
    other_deps={
        "_integ_eol_recycling_rates_minerals_base_metals": {
            "initial": {
                "current_eol_rr_minerals_base_metals": 1,
                "all_minerals_virgin_base_metals": 1,
            },
            "step": {"improvement_recycling_rates_minerals_base_metals": 1},
        }
    },
)
def eol_recycling_rates_minerals_base_metals():
    """
    Recycling rates minerals for the rest of the economy.
    """
    return _integ_eol_recycling_rates_minerals_base_metals()


_integ_eol_recycling_rates_minerals_base_metals = Integ(
    lambda: improvement_recycling_rates_minerals_base_metals(),
    lambda: current_eol_rr_minerals_base_metals() * all_minerals_virgin_base_metals(),
    "_integ_eol_recycling_rates_minerals_base_metals",
)


@component.add(
    name="fraction of the decrease Cu ocean profit delay",
    units="DMNL",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="GROWTH MATERIAL INTENSITIES SP",
    units="DMNL/Year",
    subscripts=["METALS W I"],
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_growth_material_intensities_sp"},
)
def growth_material_intensities_sp():
    """
    Growth rate of Metal intensities for Copper, Aluminium, Iron and Nickel.
    """
    return _ext_constant_growth_material_intensities_sp()


_ext_constant_growth_material_intensities_sp = ExtConstant(
    "scenario_parameters/scenario_parameters.xlsx",
    "materials",
    "GROWTH_MATERIAL_INTENSITIES_SP*",
    {"METALS W I": _subscript_dict["METALS W I"]},
    _root,
    {"METALS W I": _subscript_dict["METALS W I"]},
    "_ext_constant_growth_material_intensities_sp",
)


@component.add(
    name="HISTORICAL Cu DEMAND",
    units="Mt/Year",
    comp_type="Data",
    comp_subtype="External",
    depends_on={
...
@component.add(
    name="IMPLICIT PRICE MATERIALS Cu",
    units="Mdollars 2015/Mt",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "time": 1,
...
@component.add(
    name="improvement recycling rates minerals base metals",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def improvement_recycling_rates_minerals_base_metals():
    """
    ...
    return (
        if_then_else(
            ...
            lambda: if_then_else(
                select_mineral_rr_targets_sp_w_base_metals() == 2,
                lambda: common_rr_minerals_variation_base_metals()
                * eol_recycling_rates_minerals_base_metals(),
                lambda: by_mineral_rr_variation_metals(),
            ),
        )
        * constrain_rr_improv_for_per_mineral_base_metals()
    )
...
@component.add(
    name="increase Cu cumulative mining",
    units="Mt/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="increase delayed TS Cu ocean profit delay",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="increase delayed TS Cu profit",
    units="M$/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="INITIAL OUTPUT REAL MATERIALS",
    units="Mdollars 2015/Year",
...
@component.add(
    name="Loss during separtion and proccessing",
    units="Mt/Year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
@component.add(
    name="material intensities variation",
    units="DMNL",
    subscripts=["METALS W I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def material_intensities_variation():
    """
    Metal Intensities variations for copper, aluminium, iron and nickel multiplied by timestep.
    """
    value = xr.DataArray(
        np.nan, {"METALS W I": _subscript_dict["METALS W I"]}, ["METALS W I"]
    )
    value.loc[["Cu W"]] = if_then_else(
        select_material_intensitiy_sp() == 1,
        lambda: float(growth_material_intensities_sp().loc["Cu W"]) * time_step(),
        lambda: 0,
    )
    value.loc[["Al W"]] = if_then_else(
        select_material_intensitiy_sp() == 1,
        lambda: float(growth_material_intensities_sp().loc["Al W"]) * time_step(),
        lambda: 0,
    )
    value.loc[["Fe W"]] = if_then_else(
        select_material_intensitiy_sp() == 1,
        lambda: float(growth_material_intensities_sp().loc["Fe W"]) * time_step(),
        lambda: 0,
    )
    value.loc[["Ni W"]] = if_then_else(
        select_material_intensitiy_sp() == 1,
        lambda: float(growth_material_intensities_sp().loc["Ni W"]) * time_step(),
        lambda: 0,
    )
    return value
...
@component.add(
    name="Max recycling rates minerals base metals",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_constant_max_recycling_rates_minerals_base_metals"
    },
)
def max_recycling_rates_minerals_base_metals():
    """
    Maximum assumed recycling rate per mineral
    """
    return _ext_constant_max_recycling_rates_minerals_base_metals()


_ext_constant_max_recycling_rates_minerals_base_metals = ExtConstant(
    "model_parameters/materials/materials.xlsx",
    "World",
    ...
    _root,
    {},
    "_ext_constant_max_recycling_rates_minerals_base_metals",
)


@component.add(
    name="mining Cu known reserves",
    units="Mt/Years",
    subscripts=["ORE GRADES I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def mining_cu_known_reserves():
    ...
    value.loc[["RICH GRADE"]] = np.maximum(
        float(cu_known_reserves().loc["RICH GRADE"])
    ...
    value.loc[["HIGH GRADE"]] = np.maximum(
        high_grade_factor()
        * float(cu_known_reserves().loc["HIGH GRADE"])
        * cu_extraction_coefficient()
        * float(change_cu_grade().loc["HIGH GRADE"]),
        0,
    )
    value.loc[["LOW GRADE"]] = np.maximum(
        float(change_cu_grade().loc["LOW GRADE"])
        * float(cu_known_reserves().loc["LOW GRADE"])
        * cu_extraction_coefficient(),
        0,
    )
    value.loc[["ULTRALOW GRADE"]] = np.maximum(
        float(change_cu_grade().loc["ULTRALOW GRADE"])
        * cu_extraction_coefficient()
        * float(cu_known_reserves().loc["ULTRALOW GRADE"])
        * float(change_cu_mining_technology_s_curve().loc["ULTRALOW GRADE"]),
        0,
    )
    value.loc[["TRACE GRADE"]] = np.maximum(
        float(change_cu_grade().loc["TRACE GRADE"])
        * float(cu_known_reserves().loc["TRACE GRADE"])
        * float(change_cu_mining_technology_s_curve().loc["TRACE GRADE"])
        * cu_extraction_coefficient(),
        0,
    )
    value.loc[["OCEANS GRADE"]] = np.maximum(
        ...
        * cu_extraction_coefficient()
        * float(cu_known_reserves().loc["OCEANS GRADE"]),
        0,
    )
    return value
...
@component.add(
    name="RR MINERALS ALT TECHN RES VS TOTAL ECONOMY BASE METALS",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_constant_rr_minerals_alt_techn_res_vs_total_economy_base_metals"
    },
)
def rr_minerals_alt_techn_res_vs_total_economy_base_metals():
    """
    ...
    """
    return _ext_constant_rr_minerals_alt_techn_res_vs_total_economy_base_metals()


_ext_constant_rr_minerals_alt_techn_res_vs_total_economy_base_metals = ExtConstant(
    "model_parameters/materials/materials.xlsx",
    "World",
    ...
    _root,
    {},
    "_ext_constant_rr_minerals_alt_techn_res_vs_total_economy_base_metals",
)


@component.add(
    name="SELECT MATERIAL INTENSITIY SP",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_select_material_intensitiy_sp"},
)
def select_material_intensitiy_sp():
    """
    This select can take two values: 0: Material input intensity coefficients remain constant 1: Material input intensity coefficients improve for the chosen sectors by an annual rate (historical trends)
    """
    return _ext_constant_select_material_intensitiy_sp()


_ext_constant_select_material_intensitiy_sp = ExtConstant(
    "scenario_parameters/scenario_parameters.xlsx",
    "materials",
    "SELECT_MATERIAL_INTENSITIY_SP",
    {},
    _root,
    {},
    "_ext_constant_select_material_intensitiy_sp",
)


@component.add(
    name="sum hidden resources",
    units="Mt",
    subscripts=["ORE GRADES I"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def sum_hidden_resources():
    ...
    return xr.DataArray(
        sum(
            cu_hidden_resources().rename({"ORE GRADES I": "ORE GRADES I!"}),
            dim=["ORE GRADES I!"],
        ),
...
def sum_known_reserves():
    ...
    return xr.DataArray(
        sum(
            cu_known_reserves().rename({"ORE GRADES I": "ORE GRADES I!"}),
            dim=["ORE GRADES I!"],
        ),
        {"ORE GRADES I": _subscript_dict["ORE GRADES I"]},
        ["ORE GRADES I"],
...
@component.add(
    name="SWITCH Cu RECYCLING POLICY",
    units="DMNL",
    comp_type="Constant",
    comp_subtype="Normal",
)
...
def total_initial_cu_urr():
    ...
    """
    return sum(
        initial_cu_hidden_resources().rename({"ORE GRADES I": "ORE GRADES I!"})
        + initial_cu_known_reserves().rename({"ORE GRADES I": "ORE GRADES I!"}),
        dim=["ORE GRADES I!"],
    )


@component.add(
    name="Total supply from scrap old and new",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def water_cu_extraction():
    ...
    """
    return (
        float(water_use_cu().loc["TRACE GRADE"])
        + float(water_use_cu().loc["ULTRALOW GRADE"])
        + float(water_use_cu().loc["LOW GRADE"])
        + float(water_use_cu().loc["HIGH GRADE"])
        + float(water_use_cu().loc["RICH GRADE"])
        + water_use_cu_secondary()
    )


@component.add(
    name="WATER FORECAST COCHILICO FOR CHILE",
    units="Mm3/Years",
    comp_type="Auxiliary",
    comp_subtype="with Lookup",
    depends_on={"time": 1},
...
def water_use_cu():
    ...
    value.loc[["RICH GRADE"]] = (
        float(mining_cu_known_reserves().loc["RICH GRADE"])
        * float(water_use_per_cu().loc["RICH GRADE"])
        * unit_conversion_mm3_m3()
        * unit_conversion_t_mt()
    )
    value.loc[["HIGH GRADE"]] = (
        float(mining_cu_known_reserves().loc["HIGH GRADE"])
        * float(water_use_per_cu().loc["HIGH GRADE"])
        * unit_conversion_mm3_m3()
        * unit_conversion_t_mt()
    )
    value.loc[["LOW GRADE"]] = (
        float(water_use_per_cu().loc["LOW GRADE"])
        * float(mining_cu_known_reserves().loc["LOW GRADE"])
        * unit_conversion_mm3_m3()
        * unit_conversion_t_mt()
    )
    value.loc[["ULTRALOW GRADE"]] = (
        float(mining_cu_known_reserves().loc["ULTRALOW GRADE"])
        * float(water_use_per_cu().loc["ULTRALOW GRADE"])
        * unit_conversion_mm3_m3()
        * unit_conversion_t_mt()
    )
    value.loc[["TRACE GRADE"]] = (
        float(water_use_per_cu().loc["TRACE GRADE"])
        * float(mining_cu_known_reserves().loc["TRACE GRADE"])
        * unit_conversion_mm3_m3()
        * unit_conversion_t_mt()
    )
...
@component.add(
    name="water use Cu mining and recycling",
    units="Mm3/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def water_use_per_cu():
    ...
    value.loc[["RICH GRADE"]] = float(water_per_cu().loc["RICH GRADE"])
    value.loc[["HIGH GRADE"]] = float(water_per_cu().loc["HIGH GRADE"])
    value.loc[["LOW GRADE"]] = float(water_per_cu().loc["LOW GRADE"])
    value.loc[["ULTRALOW GRADE"]] = float(water_per_cu().loc["ULTRALOW GRADE"])
    value.loc[["TRACE GRADE"]] = float(water_per_cu().loc["TRACE GRADE"])
    value.loc[["OCEANS GRADE"]] = 0
    return value


@component.add(
    name="WORLD TOTAL ENERGY CONSUMPTION OF Cu MINING HISTORICAL",
    units="TJ/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
@component.add(
    name="WORLD WATER FORCAST COCHILICO",
    units="Mm3/Years",
    comp_type="Auxiliary",
    comp_subtype="Normal",
