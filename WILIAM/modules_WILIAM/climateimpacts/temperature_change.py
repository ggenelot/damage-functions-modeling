"""
Module climateimpacts.temperature_change
...
@component.add(
    name="average temperature change from 2015",
    units="DegreesC",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
@component.add(
    name="delayed 20 temperature change from 2015",
    units="DegreesC",
    comp_type="Stateful",
...
def ecs():
    """
    Equilibrioum climate sensititivy based on the “input method selected”, i.e., exogenous constant chosen, or ECDF (Empirical cumulative distribution function ) from AR5. This is the amount of global warming after a doubling of the atmospheric CO2 concentration. There is a high uncertainty about this parameter.)
    """
    return if_then_else(
        switch_model_explorer() == 1,
...
_integ_heat_in_deep_ocean = Integ(
    lambda: heat_transfer().loc[_subscript_dict["upper"]].rename({"LAYERS I": "upper"})
    - xr.DataArray(
        heat_transfer()
    ...
    * deep_ocean_heat_cap().loc[_subscript_dict["upper"]].rename({"LAYERS I": "upper"}),
    "_integ_heat_in_deep_ocean",
)
...
def probabilistic_ecs():
    """
    Sampled Equilibrium Climate Sensitivity (ECS) based on Empirical cumulative distribution function (ECDF) as estimated by Rogelj et al (2014) and used in the IPCC AR5 WGIII
    """
